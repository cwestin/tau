...
pxHasher *pxHasherCreate(pxAlloc *pAlloc, pxInterface *pOwner)
{
    ...
    pHasher->hash = 0xca5015ac;
    pxObjectStructInit(&pHasher->objectStruct, &pxHasherObjectVt, pOwner);

    return (pxHasher *)&pHasher->objectStruct.pObjectVt;
}

static const unsigned pxHasher_random[]; // initialized below
...
void PXHASHER_hashBytes(pxHasher *pI, const void *p, size_t length)
{
    ...
    for(const unsigned char *pc = (const unsigned char *)p;
        length; ++pc, --length)
    {
        pThis->hash ^= pxHasher_random[*pc];

        // rotate left 5 bits
        pThis->hash = (pThis->hash << 5) | (((unsigned)pThis->hash) >> 27);
    }
}


// generated by the program below
static const unsigned pxHasher_random[] =
{
    0x379abbb2,
    0x55893d4b,
    0xf20b349b,
